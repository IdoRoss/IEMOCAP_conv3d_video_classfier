...
def main():
    ...
    if transfare:
        ...
        #transfare_model = load_model("models/transfare_model_1.h5")
        train_c3d_model(transfare_model, "transfare_model_1", NUM_EPOCHS, train, valid)
        test_model(transfare_model,"transfare_model_1", valid, classes)

    if groundup:
        groundup_model = C3D_groundup_model(len(classes))
        # groundup_model = load_model("models/groundup_model_1.h5")
        train_c3d_model(groundup_model, "groundup_model_1", NUM_EPOCHS, train, valid)
        test_model(groundup_model,"groundup_model_1", valid, classes)
